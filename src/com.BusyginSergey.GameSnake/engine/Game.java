package com.BusyginSergey.GameSnake.engine;
...
public class Game extends Application implements GameScreen {
        ...
        for(int y = 0; y < this.height; ++y) {
            for(int x = 0; x < this.width; ++x) {
                ...
                if (this.showGrid && children.size() > 0) {
                    ...
                    cell.setStroke(this.toFXColor(com.BusyginSergey.GameSnake.engine.Color.BLACK));
                }
    ...
    private void createScorePanel() {
        ...
        this.scoreText.setFill(this.toFXColor(com.BusyginSergey.GameSnake.engine.Color.BLACK));
        StackPane scorePane = new StackPane(new Node[]{this.scoreText});
        scorePane.setBorder(new Border(new BorderStroke[]{new BorderStroke(this.toFXColor(com.BusyginSergey.GameSnake.engine.Color.BLACK), BorderStrokeStyle.SOLID, CornerRadii.EMPTY, BorderWidths.DEFAULT)}));
        scorePane.setLayoutY((double)(this.height * cellSize + 110 + 6));
        int scoreHeight = 20;
        Rectangle rectangle;
        if (this.showGrid) {
            rectangle = new Rectangle((double)((this.width * cellSize - 1) / 2), (double)scoreHeight, this.toFXColor(com.BusyginSergey.GameSnake.engine.Color.WHITE));
            scorePane.setLayoutX((double)(125 + (this.width * cellSize - 1) / 4));
        } else {
            rectangle = new Rectangle((double)(this.width * cellSize / 2), (double)scoreHeight, this.toFXColor(com.BusyginSergey.GameSnake.engine.Color.WHITE));
    ...
    public void setCellColor(int x, int y, com.BusyginSergey.GameSnake.engine.Color color) {
        if (color != null && color != com.BusyginSergey.GameSnake.engine.Color.NONE) {
            ObservableList<Node> children = this.cells[y][x].getChildren();
    ...
    public com.BusyginSergey.GameSnake.engine.Color getCellColor(int x, int y) {
        ObservableList<Node> children = this.cells[y][x].getChildren();
        if (children.size() > 0) {
            Color color = (Color)((Rectangle)children.get(0)).getFill();
            return this.fromFXColor(color);
        } else {
            return com.BusyginSergey.GameSnake.engine.Color.NONE;
        }
    }
    ...
    public void setCellTextColor(int x, int y, com.BusyginSergey.GameSnake.engine.Color color) {
        ObservableList<Node> children = this.cells[y][x].getChildren();
        if (children.size() > 1) {
            Text text = (Text)children.get(1);
    ...
    public com.BusyginSergey.GameSnake.engine.Color getCellTextColor(int x, int y) {
        ObservableList<Node> children = this.cells[y][x].getChildren();
        if (children.size() > 1) {
            Text text = (Text)children.get(1);
    ...
    private com.BusyginSergey.GameSnake.engine.Color fromFXColor(Color color) {
        ...
        com.BusyginSergey.GameSnake.engine.Color[] result = new com.BusyginSergey.GameSnake.engine.Color[]{com.BusyginSergey.GameSnake.engine.Color.NONE};
        optionalColorEntry.ifPresent((entry) -> {
            result[0] = com.BusyginSergey.GameSnake.engine.Color.valueOf((String)entry.getKey());
        });
        return result[0];
    }

    private Color toFXColor(com.BusyginSergey.GameSnake.engine.Color color) {
        if (color == com.BusyginSergey.GameSnake.engine.Color.NONE) {
    ...
    public void setCellValueEx(int x, int y, com.BusyginSergey.GameSnake.engine.Color cellColor, String value) {
        this.setCellValue(x, y, value);
        this.setCellColor(x, y, cellColor);
    }

    public void setCellValueEx(int x, int y, com.BusyginSergey.GameSnake.engine.Color cellColor, String value, com.BusyginSergey.GameSnake.engine.Color textColor) {
        this.setCellValueEx(x, y, cellColor, value);
        this.setCellTextColor(x, y, textColor);
    }

    public void setCellValueEx(int x, int y, com.BusyginSergey.GameSnake.engine.Color cellColor, String value, com.BusyginSergey.GameSnake.engine.Color textColor, int textSize) {
        this.setCellValueEx(x, y, cellColor, value, textColor);
        this.setCellTextSize(x, y, textSize);
    }

    public void showMessageDialog(com.BusyginSergey.GameSnake.engine.Color cellColor, String message, com.BusyginSergey.GameSnake.engine.Color textColor, int textSize) {
    ...
    public void setLives(int lives) {
        if (this.livesText == null) {
            ...
            this.livesText.setFill(this.toFXColor(com.BusyginSergey.GameSnake.engine.Color.ORANGE));
